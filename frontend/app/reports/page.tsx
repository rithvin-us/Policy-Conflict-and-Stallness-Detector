...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-100">Audit Reports</h1>
        ...
        <div className="space-y-2">
          {(reports.data?.items || []).map((r) => (
            <div key={r.id} className="flex items-center gap-3 rounded-lg border border-white/5 bg-ink-850/50 px-3 py-2.5">
              ...
              <span className="chip bg-white/5 text-slate-400 ring-1 ring-white/10">{r.format}</span>
              <span className="text-xs text-slate-500">{new Date(r.generated_at).toLocaleString()}</span>
              <span className="mono ml-auto text-[0.66rem] text-slate-600">{r.id}</span>
          ...
          {reports.data && reports.data.items.length === 0 && (
            <div className="py-6 text-center text-sm text-slate-600">No reports generated yet.</div>
          )}
        </div>
      </Panel>
    </div>
