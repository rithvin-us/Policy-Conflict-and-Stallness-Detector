  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-xl font-semibold text-black">Conflicts & Redundancies</h1>
              ...
              {rows.map((c) => (
                <tr key={c.id} className="group border-t border-white/5 hover:bg-white/[0.03]">
                  ...
                  <td className="px-3 py-3">
                    <span className="mono text-black">{c.policy_a_id}</span>
                    <span className="px-1 text-black">↔</span>
                    <span className="mono text-black">{c.policy_b_id}</span>
                  </td>
                  <td className="max-w-md px-3 py-3">
                    <Link href={`/conflicts/${c.id}`} className="block truncate text-black group-hover:text-accent-soft">
                      {c.explanation}
                    </Link>
                  </td>
                  <td className="px-3 py-3 text-right mono text-black">{Math.round(c.confidence * 100)}%</td>
                  <td className="px-3 py-3 text-right mono font-semibold text-black">{Math.round(c.risk)}</td>
                </tr>
              ))}
              {rows.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-black">No findings match the filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
